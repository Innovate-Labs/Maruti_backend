...
export const SupervisorData = {
  AddSupervisor: TryCatch(
    async (req: Request, res: Response, next: NextFunction) => {
      ...
      const getAllLinedata =
        await SupervisorServices.supervisorServicesData.GetAllLineByShopIds(
          shopId
        );
  ...
  EditSupervisor: TryCatch(
    async (req: Request, res: Response, next: NextFunction) => {
      const { id } = req.params;
      const { shopId, lineId, ...updateFields } = req.body;
      const data =
        await SupervisorServices.supervisorServicesData.UpdateSupervisor(
          id,
          updateFields
        );
      await SupervisorServices.supervisorServicesData.UpdateShopLine(
        lineId,
        shopId,
        id // superviseId
      );
